...
export default function ModalItem() {
    const {item, handleModal, handleAgregarPedido, pedido} = useQuiosco()
    ...
    useEffect(() => {
      if(pedido.some(itemState => itemState.id === item.id)){
        ...
        const cantidadActuali = pedido.find(itemState => itemState.id === item.id)
        setEditarCantidad(true)  
        setCantidad(cantidadActuali.cantidad)
         ...
         <div className="flex justify-end">
          ...
          <button type="button"
             className="bg-indigo-600 hover:bg-indigo-800 px-5 py-2 mt-5 text-white
              ...
              onClick={() => handleAgregarPedido({...item, cantidad})}>
              {editarCantidad ? 'Guardar Cambios' : 'Añadir al Pedido'}
          </button>
        </div>
    </div>
  )
